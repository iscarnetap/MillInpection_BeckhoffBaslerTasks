...
namespace Inspection
{
    public partial class frmBeckhoff : Form
    {
        ...
        // Configure native AOI on the camera (stop grabbing before calling).
        static void ConfigureAOI(Camera camera, int x, int y, int width, int height)
        {
                            ...
                            if (!task.Result.berr)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (err > 3)
                                    {
                                        ...
                                        bWeldonCycle = false;
                                        frmMainInspect.StopCycle = true;
                                        MessageBox.Show(task.Result.sException, "Snap+Detect4", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
                                        rep.result = false;
                                        _ = Task.Run(() => ShowForms(true));
                                        return rep;
                                    }
