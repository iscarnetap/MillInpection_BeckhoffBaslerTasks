...
namespace Inspection
{
    public partial class frmBeckhoff : Form
    {
        ...
        Bitmap bmpSave;
        public async Task<TasksParameters_Snap> Snap(bool bSnap = true, bool diam = false)
        ...
        public async Task<TasksParameters_GetLine> ProcDetect(bool bSnap = true, bool diam = false) //public bool GetLine(out bool berr, out double yc, out double Angle, bool bSnap = true)
        {
        ...
        public async Task<WebComm.CommReply> CycleInspect()
        {
            ...
            try
            {
                ...
                {
                    var task3 = Task.Run(() => SaveRejectsTop());
                    await task3;
            ...
            try
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    for (int j = 0; j < ss.Length - 1; j++) {
                        ...
                        Double x00 = (Single)(X0 * frmMainInspect.scaleW1);// * ((Single)pctSnap.Width/(scalew * w));
                        Double y00 = (Single)(Y0 * frmMainInspect.scaleH1);// * ((Single)pctSnap.Height / (scalew * h));
                        //w = (Single)W * wroi / (Single)pctSnap.Image.Width;
                        //h = (Single)H * hroi / (Single)pctSnap.Image.Height;
                        ...
                        {
                            panel4.AutoScroll = true;
                            panel4.AutoScrollMinSize = new Size(panel4.Width - 5, panel4.Height - 5);
                        ...
                        else
                        {
                            if (chkStretchImageFront_unchecked)
                            {
                                pctSnapFront.SizeMode = PictureBoxSizeMode.AutoSize;
                                panel4.AutoScroll = true;
                                panel4.AutoScrollMinSize = new Size(pctSnapFront.Width, pctSnapFront.Height);
                                if (pt.X != 0 && pt.Y != 0)
                                {
