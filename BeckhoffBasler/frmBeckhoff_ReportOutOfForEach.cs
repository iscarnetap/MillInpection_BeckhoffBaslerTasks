...
namespace Inspection
{
    public partial class frmBeckhoff : Form
    {
        ...
        string cmdpr = @"C:\Project\Cam2\Cam2BaslerML\Cam2BaslerML\bin\Debug\Cam2BaslerML.exe";
        int pctSnapH = 0;
        int pctSnapW = 0;
        int panel2H = 0;
        int panel2W = 0;
        private async void frmBeckhoff_Load(object sender, EventArgs e)
        {
            bool bCam2 = false;
            try
            {
                ...
                if (nExposure1 != 0) trkExp1.Value = nExposure1;
                ...
                string sML = @"C:\Project\Cam2\Cam2BaslerML\Cam2BaslerML\bin\Debug\ML.bat";
                if (File.Exists(sML))
                {
                    //attach camFront
        ...
        private void Detach()
        {
            try
            {
                ...
                int px = panelCamFront.PointToScreen(Point.Empty).X;
                int py = panelCamFront.PointToScreen(Point.Empty).Y;
        ...
        private void AddWindow(bool min = false)
        {
            try
            {
                ...
                SetWindowLong(hwndCamFront, GWL_STYLE, (uint)lStyle);
                if(min)
        ...
        public async void LoadIni(int nCam) //, bool bLoadExposure)
        {
            ...
            if (nCam == 1) //Top
            {
                ...
                if (oIniFile["Camera2"]["Exposure"] != null && oIniFile["Camera2"]["Exposure"].Trim() != "")
                {
                    nExposure1 = Convert.ToInt32(Convert.ToDouble(oIniFile["Camera2"]["Exposure"]));
                    trkExp1.Value = nExposure1;
...
*/
                            ...
                            if (chkShowCross.Checked || chkUseSearchArea.Checked)
                            {
                                using (Graphics g = Graphics.FromImage(pct1liveTab.Image))
                                {
                                    ...
                                    if (chkUseSearchArea.Checked && w > 0 && h > 0)
                                    {
                                        ...
                                        Point FirstPoint = new Point(Convert.ToInt16(txtRectPointX.Text), Convert.ToInt16(txtRectPointY.Text));
                                        Point SecondPoint = new Point(Convert.ToInt16(txtRectPointX.Text) + Convert.ToInt16(txtSearchAreaWidth.Text),
        ...
        private async void btnSnap_Click(object sender, EventArgs e)
        {
           try
            ...
            try
            {
                ...
                //if (nCam == 1 && !(pct1liveTab.Image is null))
                //    pct1liveTab.Image.Save(aPath + "\\Images\\Onesnap"  + ".jpg", ImageFormat.Jpeg);
                //for (int i = 0; i < frmMainInspect.SnapFile.Length; i++) frmMainInspect.SnapFile[i] = "";
                //frmMainInspect.SnapFile[0] = aPath + "\\Images\\OneSnap"  + ".jpg";
                //else if (nCam == 2)
                //    pct1liveTab.Image.Save(aPath + "\\Images\\snap2.jpg", ImageFormat.Jpeg);
            }
            catch (Exception exception)
            {
                string source = "";
        ...
        Bitmap bmpSave;
        public async Task<TasksParameters_Snap> Snap(bool bSnap = true)
        ...
        {
            TasksParameters_Snap GetParamsSnap;
            if (!chkSaveFile.Checked)
            {
                //pct1liveTab.Image.Save(ms, ici, myEncoderParameters); //jgpEncoder
                frmMainInspect.SaveOnDisk = false;
            }
            else
            {
                //pct1liveTab.Image.Save(frmMainInspect.mems[opt - 1], ici, myEncoderParameters); //jgpEncoder
                frmMainInspect.SaveOnDisk = true;
            }
            ...
            if (bSnap && camera1 != null && camera1.IsOpen && !(pct1liveTab.Image is null))
            {
                ...
                if (true || (bool)inv.get(chkSaveFile, "Checked"))
                {
                    try
                    {
                        ...
                        {
                            //pct1liveTab.Image.Save(aPath + "\\Images\\snap.jpg", jgpEncoder, myEncoderParameters); //ImageFormat.Jpeg);

                            //if (form2.optP2.Checked || form2.optP3.Checked || form2.optP4.Checked) {
                            //    pct1liveTab.Image.RotateFlip(RotateFlipType.Rotate180FlipNone);
                            //}
                            //for(int i=0; i< frmMainInspect.mems.Length; i++)
                            //{
                            //    if (frmMainInspect.mems[i]!=null && !frmMainInspect.mems[i].CanWrite) frmMainInspect.mems[i].Close();
                            //    else if (frmMainInspect.mems[i] == null) frmMainInspect.mems[i] = new System.IO.MemoryStream();
                            //}
                            ...
                            if (GetParamsSnap.width == 0 || GetParamsSnap.height == 0)
                            {
                                ...
                                //pct1liveTab.Image.Save(ms, ici, myEncoderParameters); //jgpEncoder
                                //bmpSave = (Bitmap)pct1liveTab.Image;
                                
                                //int ind = opt - 1;
                                //Bitmap btm = (Bitmap)pct1liveTab.Image.Clone();
                               
                                //frmMainInspect.SnapBitmap[ind] = btm;
                            } 
                            else
                            {
                                ...
                                //bmpSave.RotateFlip(RotateFlipType.Rotate270FlipNone);

                                bmpSave.Save(ms, ici, myEncoderParameters); //jgpEncoder
                                //MemoryStream ms1 = new MemoryStream();
                                //bmpSave.Save(ms1, ImageFormat.Png); //pngEncoder
                                //frmMainInspect.Mstream[opt - 1] = ms;
                                //ms.CopyTo(frmMainInspect.Mstream[opt - 1]);
                                //bmpSave.Save(frmMainInspect.mems[opt - 1], ici, myEncoderParameters); //jgpEncoder
                                g.Dispose();
                            }
                            ...
                            using (FileStream file = new FileStream(aPath + "\\Images\\snap" + opt.ToString()+ ".jpg", FileMode.Create, FileAccess.Write, FileShare.Inheritable))
                            {
                                if (chkSaveFile.Checked)
                                {
                                    ...
                                    //for (int i = 0; i < frmMainInspect.SnapFile.Length; i++) frmMainInspect.SnapFile[i] = "";
                                    //if (opt > 0) frmMainInspect.SnapFile[opt - 1] = aPath + "\\Images\\snap" + opt.ToString() + " " + nFrameMax.ToString() + ".jpg";
                                    //else frmMainInspect.SnapFile[0] = aPath + "\\Images\\snap" + opt.ToString() + " " + nFrameMax.ToString() + ".jpg";
                                ...
                                else
                                {
                                    ...
                                    //NPNP
                                    Bitmap btm = (Bitmap)pct1liveTab.Image.Clone();
                                    //Bitmap btm = new Bitmap(@"c:\Project\Cam2\Cam2BaslerML\Cam2BaslerML\bin\Debug\Images\snap-inspect.jpg");
                                    frmMainInspect.SnapBitmap[ind] = btm;
                                    pct1liveTab.Image.Save(ms, ici, myEncoderParameters);
                                    frmMainInspect.Mstream[ind] = ms;
                                    //frmMainInspect.SnapBitmap[ind].Save(ms, System.Drawing.Imaging.ImageFormat.Png);
                        ...
                        var task0 = Task.Run(() => frmMainInspect.ImageFromFile(opt - 1));
                        await task0;

                        //pct1liveTab.Image.Save(aPath + "\\Images\\snap " + DateTime.Now.ToString("yy-MM-dd HH-mm-ss") + ".jpg",
                        //    jgpEncoder, myEncoderParameters); //ImageFormat.Jpeg);

                        ////2. Save left part
                        //Bitmap bmp = new Bitmap((int)(pct1liveTab.Image.Width * 0.6), pct1liveTab.Image.Height);
                        //Graphics g = Graphics.FromImage(bmp);
                        //g.DrawImage(pct1liveTab.Image, new System.Drawing.Rectangle(0, 0, (int)(pct1liveTab.Image.Width * 0.6), pct1liveTab.Image.Height), 
                        //    new System.Drawing.Rectangle(0, 0, (int)(pct1liveTab.Image.Width * 0.6), pct1liveTab.Image.Height), System.Drawing.GraphicsUnit.Pixel);
                        //bmp.Save(aPath + "\\Images\\snap1.jpg", jgpEncoder, myEncoderParameters);    //ImageFormat.Jpeg);

                        ////3. Save right part
                        //g.DrawImage(pct1liveTab.Image, new System.Drawing.Rectangle(0, 0, (int)(pct1liveTab.Image.Width * 0.6), pct1liveTab.Image.Height),
                        //    new System.Drawing.Rectangle((int)(pct1liveTab.Image.Width * 0.4), 0, (int)(pct1liveTab.Image.Width * 0.6), pct1liveTab.Image.Height), System.Drawing.GraphicsUnit.Pixel);
                        //bmp.Save(aPath + "\\Images\\snap2.jpg", jgpEncoder, myEncoderParameters);    //ImageFormat.Jpeg);

                        ////4. Save resized 2048x2048
                        //bmp = new Bitmap(2048, 2048);
                        //g = Graphics.FromImage(bmp);
                        //g.DrawImage(pct1liveTab.Image, new System.Drawing.Rectangle(0, 0, 2048, 2048),
                        //    new System.Drawing.Rectangle(0, 0, pct1liveTab.Image.Width, pct1liveTab.Image.Height), 
                        //    System.Drawing.GraphicsUnit.Pixel);
                        //bmp.Save(aPath + "\\Images\\snap3.jpg", jgpEncoder, myEncoderParameters);    //ImageFormat.Jpeg);
        ...
        public async Task<TasksParameters_GetLine> ProcDetect(bool bSnap = true) //public bool GetLine(out bool berr, out double yc, out double Angle, bool bSnap = true)
        {
            TasksParameters_GetLine GetParams;
            ...
            try
            {
                ...
                //frmBeckhoff.mFormBeckhoffDefInstance.txtAcqTime.Invoke((Action)(() => { frmBeckhoff.mFormBeckhoffDefInstance.txtAcqTime.Text = ""; }));
                //frmBeckhoff.mFormBeckhoffDefInstance.txtInspectTime.Invoke((Action)(() => { frmBeckhoff.mFormBeckhoffDefInstance.txtInspectTime.Text = ""; }));
                //frmBeckhoff.mFormBeckhoffDefInstance.txtElapsedTime.Invoke((Action)(() => { frmBeckhoff.mFormBeckhoffDefInstance.txtElapsedTime.Text = ""; }));
                long t = stopwatch.ElapsedMilliseconds;

                //TasksParameters_Snap GetParamsSnap;
                //GetParamsSnap.x = 0; GetParamsSnap.y = 0; GetParamsSnap.width = 0; GetParamsSnap.height = 0; GetParamsSnap.sException = "";
                //int rc = 0;
                ...
                if (bSnap) {
                    ...
                    var task = Task.Run(() => Snap(!bDebugMode)); //(bSnap)
                    await task;
                    
                    //rc = frmBeckhoff.mFormBeckhoffDefInstance.Snap(out x, out y, out width, out height, out sException);
                    if (task.Result.rc < 0) { // rc < 0
                        if (task.Result.rc == -1)
                            GetParams.sException = "Snap Error: " + task.Result.sException;
                        //MessageBox.Show("Snap Error: " + task.Result.sException, "ProcDetect", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        else if (task.Result.rc == -2)
                            GetParams.sException = "Snap Exception: " + task.Result.sException;
                        //MessageBox.Show("Snap Exception: " + task.Result.sException, "ProcDetect", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        else
                            GetParams.sException = "Snap Error";
                        //MessageBox.Show("Snap Error", "ProcDetect", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        GetParams.berr = true;
                        return GetParams;
                    }
                ...
                //frmBeckhoff.mFormBeckhoffDefInstance.txtAcqTime.Invoke((Action)(() => {
                //    frmBeckhoff.mFormBeckhoffDefInstance.txtAcqTime.Text = String.Format("{0:f0}", t1 - t);
                //}));

                var task1 = Task.Run(() => ShowImage(x, y, width, height));
                await task1;
                //rc = ShowImage(out berr, out yc, out Angle);

                if (task1.Result.berr) {
                    //if (task1.Result == -1)
                    //    MessageBox.Show("houghLineTransform: Line not found !", "ProcDetect", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
                    //else if (task1.Result == -2)
                    //    MessageBox.Show("ShowImage: Snap file not exists !", "ProcDetect", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
                    //else
                    //    MessageBox.Show("ShowImage error", "ProcDetect", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
                    GetParams.sException = task1.Result.sException;
                    GetParams.berr = true;
                    return GetParams;
                }

                //using (var file = new FileStream(frmVision.mFormVisionDefInstance.aPath + "\\Images\\snap.jpg", FileMode.Open, FileAccess.Read, FileShare.Inheritable))
                //{
                //    image = (Bitmap)Bitmap.FromStream(file); // (Bitmap)Bitmap.FromFile(image_path);
                //    processed = image;
                ...
                //    HoughLineTransformProc(); //rc = HoughLineTransformProc();
                //    //if (!rc) 
                //    //    MessageBox.Show("Line not found !", "houghLineTransform", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
                //}
                ////pctBoxLine.Invoke((Action)(() => {
                //pctBoxLine.Image = processed; // img;
                //pctBoxLine.Size = processed.Size; // img.Size;
                ////}));
                ...
                //frmBeckhoff.mFormBeckhoffDefInstance.txtInspectTime.Invoke((Action)(() => {
                //    frmBeckhoff.mFormBeckhoffDefInstance.txtInspectTime.Text = String.Format("{0:f0}", t2 - t1);
                //}));
                inv.settxt(frmBeckhoff.mFormBeckhoffDefInstance.txtElapsedTime, String.Format("{0:f0}", t2 - t));
                //frmBeckhoff.mFormBeckhoffDefInstance.txtElapsedTime.Invoke((Action)(() => {
                //    frmBeckhoff.mFormBeckhoffDefInstance.txtElapsedTime.Text = String.Format("{0:f0}", t2 - t);
                //}));
                stopwatch.Stop();
                GetParams.sException = "";
                GetParams.berr = false;
                ...
                //MessageBox.Show("Error: " + e.Message, "ProcDetect", MessageBoxButtons.OK, MessageBoxIcon.Error);
                GetParams.berr = true;
            }
        ...
        //public int ShowImage(out bool berr, out double yc, out double Angle)
        {
            ...
            if (!bDebugMode) {
                ...
                if (!frmMainInspect.SaveOnDisk)
                {
                    pctSnap.Invoke((Action)(() =>
                    {
                        pctSnap.Image = processed; // img;
                        if (nOpt > 0)
                        {
                            //frmMainInspect.SnapBitmap[nOpt - 1] = (Bitmap)pct1liveTab.Image;
                            frmMainInspect.SnapImage[nOpt - 1] = pct1liveTab.Image;
                            Bitmap bmp = new Bitmap(pct1liveTab.Image);
                            frmMainInspect.StreamImage[nOpt - 1] = new MemoryStream();

                            bmp.Save(frmMainInspect.StreamImage[nOpt - 1], System.Drawing.Imaging.ImageFormat.Png);
                            bmp.Dispose();

                            //pctSnap.Image.Save(frmMainInspect.StreamImage[nOpt - 1], ici, myEncoderParameters);
                            //pctSnap.Image.Save(frmMainInspect.StreamImage[nOpt - 1],ImageFormat.Jpeg);
                            //frmMainInspect.StreamImage[nOpt - 1] = new System.IO.MemoryStream();
                            //pctSnap.Image.Save(frmMainInspect.StreamImage[nOpt - 1], pctSnap.Image.RawFormat);
                            //pctSnap.Image.Save(frmMainInspect.StreamImage[nOpt - 1], ici, myEncoderParameters);
                        }
                        else
                        {
                            frmMainInspect.SnapImage[0] = processed;// image;
                            //frmMainInspect.StreamImage[0] = new MemoryStream();
                            //pctSnap.Image.Save(frmMainInspect.StreamImage[0], ici, myEncoderParameters);
                            //frmMainInspect.SnapBitmap[0] = (Bitmap)pct1liveTab.Image;
                            Bitmap bmp = new Bitmap(pct1liveTab.Image);
                            frmMainInspect.StreamImage[0] = new MemoryStream();

                            bmp.Save(frmMainInspect.StreamImage[nOpt - 1], System.Drawing.Imaging.ImageFormat.Png);
                        }
                        //pctSnap.Size = processed.Size; // img.Size;
                    }));
                    if (nOpt > 0) frmMainInspect.SnapImage[nOpt - 1] = processed;
                    else frmMainInspect.SnapImage[0] = processed;
                }
                ...
                //if ((bool)inv.get(opt1, "Checked")) { inv.set(pct1, "Image", processed); nOpt = 1; }
                //else if ((bool)inv.get(opt2, "Checked")) { inv.set(pct2, "Image", processed); nOpt = 2; }
                //else if ((bool)inv.get(opt3, "Checked")) { inv.set(pct3, "Image", processed); nOpt = 3; }
                //else if ((bool)inv.get(opt4, "Checked")) { inv.set(pct4, "Image", processed); nOpt = 4; }
                //else if ((bool)inv.get(opt5, "Checked")) { inv.set(pct5, "Image", processed); nOpt = 5; }
                //else if ((bool)inv.get(opt6, "Checked")) { inv.set(pct6, "Image", processed); nOpt = 6; }
                //else if ((bool)inv.get(opt7, "Checked")) { inv.set(pct7, "Image", processed); nOpt = 7; }
                //else if ((bool)inv.get(opt8, "Checked")) { inv.set(pct8, "Image", processed); nOpt = 8; }
                //else if ((bool)inv.get(opt9, "Checked")) { inv.set(pct9, "Image", processed); nOpt = 9; }
                //else if ((bool)inv.get(opt10, "Checked")) { inv.set(pct10, "Image", processed); nOpt = 10; }
                //else if ((bool)inv.get(opt11, "Checked")) { inv.set(pct11, "Image", processed); nOpt = 11; }
                //else if ((bool)inv.get(opt12, "Checked")) { inv.set(pct12, "Image", processed); nOpt = 12; }
                //else if ((bool)inv.get(opt13, "Checked")) { inv.set(pct13, "Image", processed); nOpt = 13; }
                //else if ((bool)inv.get(opt14, "Checked")) { inv.set(pct14, "Image", processed); nOpt = 14; }
                //else if ((bool)inv.get(opt15, "Checked")) { inv.set(pct15, "Image", processed); nOpt = 15; }
                //else if ((bool)inv.get(opt16, "Checked")) { inv.set(pct16, "Image", processed); nOpt = 16; }
                ...
                if (fname != "" && nOpt > 0) {
                    ...
                    //File.Copy(fname, fnamecopy, true);
                    //save image
                    ...
                    //ms.CopyTo(frmMainInspect.StreamImage[nOpt - 1]);
                    //ms.Close();
                    //ms.Dispose();
        ...
        public async Task<int> HoughLineTransformProc(TasksParameters_GetLine GetParams)
        ...
        private async Task<string> SaveSnapFile()
        {
            string sErMes = "";
            try
            {
        ...
        public async Task<WebComm.CommReply> CycleVision()
        {
            ...
            try
            {
                ...
                //mFrmMain.bContinue = false;
                //if (!mFrmMain.xContinue)
                //{
                //    //    //mFrmMain.CancelAsync = false;
                //    //    var taskMain = Task.Run(() => mFrmMain.StartTaske1());
                //    //    //await taskMain;
                //    this.Invoke(new Action(() => mFrmMain.btnBackgroundWorker_Click(mFrmMain.btnBackgroundWorker, null)));
                //    Thread.Sleep(1000);
                //}
                        ...
                        while (true)
                        {
                            ...
                            else
                            {
                                ...
                                if (err > 3)
                                {
                                    ...
                                    bWeldonCycle = false;
                                    frmMainInspect.StopCycle = true;
                                    MessageBox.Show(task.Result.sException, "Snap+Detect2", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
                                    rep.result = false;
                                    return rep;
                                }
                            }
                ...
                //run diam
                //inv.settxt(frmBeckhoff.mFormBeckhoffDefInstance.txtCycleTime, String.Format("0.0", stopwatch.ElapsedMilliseconds / 1000.0));
                //run front snap
                ...
                else { rep.result = false; return rep; }
                


                //run weldone from plc

                //wait inspection fini
                inv.settxt(txtMess, s);

                //long t3 = stopwatch.ElapsedMilliseconds;
                //stopwatch.Stop();
                //await taskInsp;
                //bool res = taskInsp.Result;
                //inv.settxt(frmBeckhoff.mFormBeckhoffDefInstance.txtCycleTime, String.Format("{0:f0}", t3 - t1));
                ////save file
                //int RejectB= frmMainInspect.RegionFound2save.Length - 1;
                //int RejectP = frmMainInspect.RegionFoundsave.Length - 1;
                ...
                //    for (int i = 0; i < frmMainInspect.RegionFound2save.Length; i++)
                //    {
                //        string ss = frmMainInspect.RegionFound2save[i];
                //        swr.WriteLine(ss);
                //    }
                //    swr.Close();
                ...
                //{
                //    StreamWriter swr = new StreamWriter(file);
                //    for (int i = 0; i < frmMainInspect.RegionFoundsave.Length; i++)
                //    {
                //        string ss = frmMainInspect.RegionFoundsave[i];
                //        swr.WriteLine(ss);
                //    }
                //    swr.Close();
        ...
        public async Task<WebComm.CommReply> CycleInspect()
        {
            ...
            {
                this.Invoke((Action)(() => { frmMainInspect.listBox1.Items.Clear(); }));
                this.Invoke((Action)(() => { frmMainInspect.listBox1.Items.Add("numBufferSize:" + numBufferSize.Value.ToString()); }));
                this.Invoke((Action)(() => { frmMainInspect.listBox1.Items.Add("Inspect Catalog:" + frmMainInspect.CmbCatNumText); }));
                this.Invoke((Action)(() => { frmMainInspect.listBox1.Items.Add("PartNum: " + txtPartNum.Text); }));
                this.Invoke((Action)(() => { frmMainInspect.listBox1.Items.Add("Order:" + txtOrder.Text); }));
                this.Invoke((Action)(() => { frmMainInspect.listBox1.Items.Add("Item:" + txtItem.Text); }));
                this.Invoke((Action)(() => { frmMainInspect.listBox1.Items.Add("Diameter:" + txtD.Text); }));
                this.Invoke((Action)(() => { frmMainInspect.listBox1.Items.Add("Length:" + txtL.Text); }));
                this.Invoke((Action)(() => { frmMainInspect.listBox1.Items.Add("LengthU: " + txtLU.Text); }));
                ...
                {
                    var task3 = Task.Run(() => SaveRejects());
                    await task3;
                }
        ...
        public async Task<WebComm.CommReply> WaitCycleInspectFini()
        {
                    ...
                    if(s != null && s.Length > 2)
                    {
                        switch (s[0])
                        {
                            ...
                            case "cmd97"://color
                                string sFilePath = "";
                                bool bres = false;
                                int snaps = (int)(numBufferSize.Value / 4);
        ...
        public async Task<StrWeldon> CalcDiamWeldon(bool bWeldon)
        {
            ...
            // Gaussian filtering
            CvInvoke.GaussianBlur(matGrayImage, matBlurImage, new Size(5, 5), 3);

            matBlurImage = 255 - matBlurImage;

            int absOffset = 3;
            Mat element = CvInvoke.GetStructuringElement(Emgu.CV.CvEnum.ElementShape.Rectangle,
                new Size(absOffset * 2 + 1, absOffset * 2 + 1), new Point(absOffset, absOffset));
            ...
            CvInvoke.MorphologyEx(matBlurImage, matBlurImage, Emgu.CV.CvEnum.MorphOp.Dilate, element,
                new Point(-1, -1), 2, Emgu.CV.CvEnum.BorderType.Default, new MCvScalar(0, 0, 0)); //(0)
            CvInvoke.MorphologyEx(matBlurImage, matBlurImage, Emgu.CV.CvEnum.MorphOp.Erode, element,
                new Point(-1, -1), 5, Emgu.CV.CvEnum.BorderType.Default, new MCvScalar(0, 0, 0)); //(0)
            ...
            for (int i = 0; i < contours.Size; i++)
            {
                if (contours[i].Size > 4)
                {
                    ...
                    if (rect.Width > 200 && rect.Height > 50) //rect.Width > 200 && rect.Height > 2
                    {
                        ...
                        CvInvoke.Line(result, new Point((int)((vertices[ind1].X + vertices[ind2].X) * 0.5f), (int)((vertices[ind1].Y + vertices[ind2].Y) * 0.5f)),
                            new Point((int)closest.X, (int)closest.Y), new MCvScalar(0, 0, 255), linewidth);
                        ...
                        CvInvoke.Line(result, new Point((int)((vertices[ind3].X + vertices[ind4].X) * 0.5f), (int)((vertices[ind3].Y + vertices[ind4].Y) * 0.5f)),
                            new Point((int)closest.X, (int)closest.Y), new MCvScalar(255, 0, 0), linewidth);
        ...
        public async Task<double> CalcWeldon()
        {
            double weldon = 0;
            inv.settxt(lblWeldonTop, "");
            inv.settxt(txtWeldonTime, "");
                ...
                else if (nTag >= 1 && nTag <= nFrameMax)
                {
                    ...
                    if (weldontop > 0) {
                        ...
                        g.DrawLine(new Pen(Color.Red, 1f),
                            new Point(px + (int)(10 * Math.Cos(angle)), py + (int)(10 * Math.Sin(angle))),
                            new Point(px - (int)(10 * Math.Cos(angle)), py - (int)(10 * Math.Sin(angle))));
                    } else {
                        //g.DrawLine(new Pen(Color.Blue, 1f), new Point(cx, cy), new Point(px, py));
                    }
        ...
        private void pctSnap_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                //pctSnap.Height = (int)(pctSnap.Width * 3648.0f / 5472.0f);
                //pctSnap.Width = (int)(pctSnap.Height / (3648.0f / 5472.0f));
                //pctSnap.SizeMode = PictureBoxSizeMode.StretchImage;
            ...
            try
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    for (int j = 0; j < ss.Length - 1; j++)
                    {
                        ...
                        Point pt = new Point((int)((x00 + x0roi - w / 2)* (Single)scalew), (int)((y00 + y0roi - h / 2)* (Single)scaleh));
                        pt.X = pt.X - 100;
                        pt.Y = pt.Y - 100;
                        ...
                        {
                            panel2.AutoScroll = true;
                            panel2.AutoScrollMinSize = new Size(panel2.Width-5, panel2.Height-5);
                        ...
                        else
                        {
                            if (chkStretchImage2_unchecked)
                            {
                                pctSnap.SizeMode = PictureBoxSizeMode.AutoSize;
                                panel2.AutoScroll = true;
                                panel2.AutoScrollMinSize = new Size(pctSnap.Width, pctSnap.Height);
                                if (pt.X != 0 && pt.Y != 0)
                                {
                                    panel2.AutoScrollPosition = pt;
                                }
        ...
        private void btnRun_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (hwndCamFront == IntPtr.Zero)
                {
                    ...
                    string s = @"C:\Project\Cam2\Cam2BaslerML\Cam2BaslerML\bin\Debug\ML.bat";
        ...
        public async Task<StrHist> CalcHistogram()
        {
            ...
            bool bShowSpline = false, bool bDoubleSize = false)
        {
            ...
            chart.Size = new Size(704, 400); //600,400
            if (bDoubleSize) chart.Size = new Size(1200, 800);
        ...
        private void SaveRejects()
        {
            try
            {
                ...
                string path = "C:\\Rejects\\"+ txtOrder.Text.Trim()+"_"+txtItem.Text.Trim()+"\\" + txtPartNum.Text.Trim() + " " + DateTime.Now.ToString("yy-MM-dd HH-mm-ss");
                ...
                using (FileStream file = new FileStream(path + "\\data" + ".txt", FileMode.Create, FileAccess.Write, FileShare.Inheritable))
                {
                    ...
                    //  swr.WriteLine(ss[i]);
                    //}
                    //swr.WriteLine("numBufferSize:" + frmMainInspect.numBufferSize.ToString());
                    //swr.WriteLine("Inspect Catalog:" + frmMainInspect.CmbCatNumText);
                    //swr.WriteLine("PartNum:" );
                    //swr.WriteLine("Order:" + txtOrder.Text);
                    //swr.WriteLine("Item:" + txtItem.Text);
                    //swr.WriteLine("Diameter:" + txtD.Text);
                    //swr.WriteLine("Length:" + txtL.Text);
                    //swr.WriteLine("LengthU:" + txtLU.Text);
                    //swr.WriteLine("LengthU:" + txtPartNum.Text);
        ...
        private void LoadRejects()
        {
            try
            {
                ...
                if (cmbDir.Text.Trim() != "")
                {
                    path = "C:\\Rejects\\" + cmbDir.Text.Trim();
                }
                else
                {
                ...
                if (File.Exists(path+"\\data.txt"))
                {
                    ...
                    foreach (string line in File.ReadLines(path+"\\data.txt"))
                    {
                        ...
                        frmMainInspect.listBox1.Items.Add(line);
                ...
                //    if (s.IndexOf("Break:") > 0 || s.IndexOf("Peel:") > 0) { ss[ss.Length - 1] = s; Array.Resize<String>(ref ss, ss.Length + 1); }
                //}
                //string path = "C:\\Rejects\\" + DateTime.Now.ToString("yy-MM-dd HH-mm-ss");
                //Directory.CreateDirectory(path);
                //using (FileStream file = new FileStream(path + "\\data" + ".txt", FileMode.Create, FileAccess.Write, FileShare.Inheritable))
                //{
                //    StreamWriter swr = new StreamWriter(file);
                ...
                //    swr.WriteLine("numBufferSize:" + frmMainInspect.numBufferSize.ToString());
                //    swr.WriteLine("Catalog:" + frmMainInspect.CmbCatNum.Text);
            ...
            try
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    for (int j = 0; j < ss.Length - 1; j++) {
                        ...
                        Double x00 = (Single)(X0 * scaleX);// * ((Single)pctSnap.Width/(scalew * w));
                        Double y00 = (Single)(Y0 * scaleY);// * ((Single)pctSnap.Height / (scalew * h));
                        //w = (Single)W * wroi / (Single)pctSnap.Image.Width;
                        //h = (Single)H * hroi / (Single)pctSnap.Image.Height;
