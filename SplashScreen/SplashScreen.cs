...
namespace Inspection
{
	...
	public partial class SplashScreen : Form
	{
        ...
        private double m_dblOpacityDecrement = 1; //.08;
		private const int TIMER_INTERVAL = 1000; //50
		...
		public SplashScreen()
		{
			InitializeComponent();
			this.Opacity = 0.0;
            if (timer_interval == 0) timer_interval = TIMER_INTERVAL;
			UpdateTimer.Interval = timer_interval; //TIMER_INTERVAL;
			UpdateTimer.Start();
        ...
        static public void ShowSplashScreen(float delay) // delay [sec]
		{
			// Make sure it's only launched once.
			if (ms_frmSplash != null)
				return;
            if (delay > 0)
                timer_interval = (int)(delay * 15);
            else
			...
			while (ms_frmSplash == null || ms_frmSplash.IsHandleCreated == false) {
                System.Threading.Thread.Sleep(timer_interval); // (TIMER_INTERVAL);
			}
		}
        ...
        // Static method called from the initializing application to give the splash
		// screen reference points. Not needed if you are using a lot of status strings.
		...
		// Function to read the checkpoint intervals from the previous invocation of the splashscreen from the XML file.
        /*
        ...
        */
		...
		// Method to store the intervals (in percent complete) from the current invocation of the splash screen to XML storage.
        /*
		...
		private void UpdateTimer_Tick(object sender, System.EventArgs e)
		{
			...
			if (m_bFirstLaunch == false && m_dblLastCompletionFraction < m_dblCompletionFraction) {
				...
				if (width > 0 && height > 0) {
                    ...
                    int iSecondsLeft = 1 + (int)(timer_interval * ((1.0 - m_dblLastCompletionFraction) / m_dblPBIncrementPerTimerInterval)) / 1000; //TIMER_INTERVAL
	...
	internal class SplashScreenXMLStorage
	{
        ...
        */ 
		...
		{
			get {
                return (ms_DefaultPercents); //return GetValue("Percents", ms_DefaultPercents); 
            }
			set {
                ms_DefaultPercents = value; //SetValue("Percents", value); 
            }
		}
		...
		{
			get {
                return ms_DefaultIncrement; //return GetValue("Interval", ms_DefaultIncrement); 
            }
			set {
                ms_DefaultIncrement = value; //SetValue("Interval", value); 
            }
		}
